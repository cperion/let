...
| Narrower integers (`U8`, `U16`) | implemented: arithmetic wraps at the width its type names, a literal adapts to a narrower operand, other mixed widths widen, and narrowing uses a checked conversion that rejects a known value that does not fit and aborts a run-time one that does not |
