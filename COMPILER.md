...
| §§7, 13.1 | Inline control and short-circuiting | If, while, break, continue, switch, and and/or become explicit blocks; arms do not become runtime closures. A loop's condition CFG belongs to its original header, not its preheader or last condition block. |
...
| §7.2 | Break and continue leave or advance the nearest loop | Each is an edge to that loop's exit or header interface, carrying the loop-carried facts and the effect thread. Leaving an iteration destroys its owned locals, exactly as reaching the body's end does. |
