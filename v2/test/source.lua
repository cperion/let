...
local V=require('v2'); local A=V.AST; local execute=require('v2.test.execute')
local count=0
...
local function run(text,n) local p=parse(text); return execute(p.bindings[1].value:build_function('example'),{n},{}) end
...
local separated=parse('let f=do first(); second() end').bindings[1].value.terminal.statements
local juxtaposed=parse('let f=do first()\nsecond() end').bindings[1].value.terminal.statements
check(#separated==2 and #juxtaposed==1 and A.Specialize:isclassof(juxtaposed[1].value))
local word=parse('let f=let x:Int let p=mark(x) let y:Int do return apply(y, let z:Int do return z+x end) end').bindings[1].value
...
local aggregate=parse('let a={let p=1; do return p end, {let left=2 let right=3}}').bindings[1].value.terminal.value
check(A.PositionalAggregate:isclassof(aggregate) and A.NamedAggregate:isclassof(aggregate.elements[2].terminal.value))
local body=parse('let f=do let x=1 target.member[index[0]]=x return x end').bindings[1].value.terminal.statements
check(#body==3 and A.Index:isclassof(body[2].place))
local text=parse('let text="é\\u{1f600}\\u{0}\\n"').bindings[1].value.terminal.value
...
print(('passed %d v2 source integration checks'):format(count))
