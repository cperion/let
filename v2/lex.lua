...
local keywords={}
for word in ('let do end own mut move return if else while switch case and or not true false'):gmatch('%S+') do keywords[word]=true end
local escapes={['\\']='\\',['"']='"',n='\n',r='\r',t='\t'}
...
function Lexer:next()
    ...
    if c:match('^[0-9]$') then
        ...
        require('v2.literal').integer(spelling,true,function(m) Lexer.fail(span,m) end)
