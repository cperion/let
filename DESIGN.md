...
Type application is **`with`** (`List with Int`), so `TypeExpr` has no `Apply` alternative: §3.1
says *"a type application is not a distinct form."* Conversions are their own choice; the previous
